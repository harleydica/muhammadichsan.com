...
Role atau peran, fitur ini sebenarnya sama saja dengan user atau user di IAM. Role dan IAM sama-sama merupakan identity yang memiliki permission terhadap aws resource, denganya kita menetapkan resource mana yang boleh di akses dan tidak boleh diakses. Perbedaan di antara keduanya ialah jikalau IAM terkait dengan suatu identitas user tertentu yang dikaitkan dengan password atau access key nah sementara itu role tidak menggunakanya.
